"""Stage 2.3 gated rerun (registered 3beb24b) — corrected footprint.
Generated from run_p5_extension.py; identical config; fig7 model.
Original docstring follows.

"""
_old_doc = """Stage 2.2 Part B — the 240 s extension run (registered:
...
CKPT_EVERY = 600.0
SEED = 1
CKPT = os.path.join(HERE, 'p5f7_resume.ckpt')
EVENTS_JSONL = os.path.join(HERE, 'p5f7_240_events_s1.jsonl')
SERIES_JSONL = os.path.join(HERE, 'p5f7_240_series_s1.jsonl')
...
def main():
    pt = build_project(dict(laterals=True, K_nearpatch=-1.10,
                            raise_oxide=0.0), fig7_corrected=True)
    ...
    while True:
        ...
        if not ckpt120 and eng.kmc_time >= 120.0:
            np.savez_compressed(
                os.path.join(HERE, 'p5f7_ckpt_kmc120.npz'),
                lattice=eng.lattice.copy(),
                procstat=eng.procstat.copy(),
                kmc_time=eng.kmc_time, kmc_step=eng.kmc_step,
                seed=SEED)
            with open(os.path.join(HERE, 'p5f7_ckpt_rng.pkl'),
                      'wb') as f:
    ...
    out = dict(mode='ON K=-1.10 raise=0.0 FIG7CORR240', seed=SEED,
    ...
    with open(os.path.join(HERE, 'p5f7_240_s1.json'), 'w') as f:
        json.dump(out, f, indent=1)
    # consolidate events for the analyzer
    events = [json.loads(x) for x in open(EVENTS_JSONL)]
    with open(os.path.join(HERE, 'p5f7_240_events_s1.json'),
              'w') as f:
